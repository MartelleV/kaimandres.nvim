...
local get_compiled_path = function()
  return table.concat({ vim.fn.stdpath("state"), "kaimandres", "compiled.lua" }, PATH_SEP)
end

function M.compile(highlights, termcolors)
  vim.loop.fs_mkdir(vim.fn.stdpath("state") .. PATH_SEP .. "kaimandres", 448)

  local fname = get_compiled_path()
  local file, err = io.open(fname, "wb")
  if not file or err then
    vim.notify("Kaimandres: Error writing " .. fname .. ":\n" .. err, vim.log.levels.ERROR)
    return
  end

  local lines = {
    "require'kaimandres'.compiled = string.dump(function()",
    "local g = vim.g",
  ...
  file:write(require("kaimandres").compiled)
  file:close()
end
